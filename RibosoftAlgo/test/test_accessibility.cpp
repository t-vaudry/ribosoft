...
TEST_CASE("Perfect accessibility", "[accessibility]") {
    float score = -1.0f;
    R_STATUS status = accessibility("CAACUGCAUGUGAUG", "cba987654..3210", ".........()....", 1.0f, 0.5f, score);
...
TEST_CASE("Imperfect accesibility", "[accessibility]") {
    ...
    R_STATUS status = accessibility("CAACUGCAUGUGAUG","cba987654..3210", "...((()((.)).).", 1.0f, 0.5f, score);
    REQUIRE(status == R_SUCCESS::R_STATUS_OK);
    REQUIRE(fabs(score - 59.1335f) < DELTA);
}

TEST_CASE("Invalid substrate sequence", "[accessibility]") {
    float score = -1.0f;
    R_STATUS status = accessibility("cjwdjvbq", "cba987654..3210", "...............", 1.0f, 0.5f, score);
...
TEST_CASE("Invalid structure length", "[accessibility]") {
    float score = -1.0f;
    R_STATUS status = accessibility("CAACUGCAUGUGAUG", "210", "...", 1.0f, 0.5f, score);
